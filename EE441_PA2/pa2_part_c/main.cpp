...
TreeNode* bstFlightPlan::AddFlight(int fnumber, TreeNode* adr)//addflight
    {
        if(adr == NULL)
        {
            adr = new TreeNode;//dynamic memory allocation for the node
...
//binary search tree in b) and prints the added flight numbers
 TreeNode *AddMonthlyNewFlights(TreeNode *&root, int *temp, int N_value){//new fligth value is N_value
...
//add the flights at current flight plan and print the added flights also
...
newflights=AddMonthlyNewRoutes(routeadd,N_value);
 cout<<" "<<endl;
BSTobject.root_tree=AddMonthlyNewFlights(BSTobject.root_tree, newflights,N_value);
  cout<<" "<<endl;
